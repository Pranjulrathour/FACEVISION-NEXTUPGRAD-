...
@patch("app.database.apply_idempotent_column_migrations")
@patch("app.database.time.sleep", return_value=None)
@patch("app.database.Base")
def test_init_db_retries_on_operational_error_then_succeeds(mock_base, mock_sleep, mock_migrate):
...
@patch("app.database.time.sleep", return_value=None)
@patch("app.database.Base")
def test_init_db_raises_after_exhausting_attempts(mock_base, mock_sleep, mock_migrate):
...
@patch("app.database.time.sleep", return_value=None)
@patch("app.database.Base")
def test_init_db_succeeds_immediately_without_retry(mock_base, mock_sleep, mock_migrate):
